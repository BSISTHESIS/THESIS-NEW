...
import com.jcreates.coffeev3.ORDERSTATUS.OrderStatus
...
class InsertNameCustomer : DialogFragment(){
    ...
    lateinit var viewModel2 :CustomerDataOrderViewModel
//    private var currentUserRepository: CurrentUserRepository? = null
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        builder.setView(inflater.inflate(R.layout.insert_new_customer, null))
        isCancelable = false
    ...
    fun UiElements(){
        ...
        viewModel = CustomerDataViewModel(requireActivity().application)
        viewModel2 = CustomerDataOrderViewModel(requireActivity().application)
...
//            header_price = et_price!!.text.toString()
            ...
            if (header_text.isEmpty()) {
                et_header!!.error = "PLEASE INPUT CUSTOMER NAME "
                Toast.makeText(getContext(), "NO  CUSTOMER NAME INPUT", Toast.LENGTH_LONG).show()
            }
...
//            }
            ...
            sweetAlertDialog.titleText = "CONTINUE  THIS CUSTOMER NAME IS CORRECT?"
            sweetAlertDialog.confirmText = "CONFIRM"
            sweetAlertDialog.setCancelable(false)
            sweetAlertDialog.setConfirmClickListener {
                sweetAlertDialog.dismissWithAnimation()
                val Locations = CustomerData(0,header_text.toString(),"UPDATE")
                viewModel.insert(Locations)
                startActivity(Intent(requireContext().applicationContext, OrderStatus::class.java))
...
//                    Handler().postDelayed({ callServer?.SendDataCoffee(getdata2()) }, 2000)
//                } else {
//                    sweetAlertDialog.dismissWithAnimation()
//                    val pDialog =
...
//        val employee_id = cursor?.getString(1)
        ...
        header.add("insert_coffee", profile_holder)
