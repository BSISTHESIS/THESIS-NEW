...
class ReceiptDialogFragment : DialogFragment() {
    ...
    var   final_change =""; var total_pay=""; var amortization = ""; var val_downpayment="";var total_dp=""; var val_monthly=""; var val_total_isv="";var val_final_amortization ="";
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        builder.setView(inflater.inflate(R.layout.receipt_dialog, null))
        isCancelable = false
    ...
    private fun Listener() {
        ...
        btn_receipt.setOnClickListener {
            ...
            if(change.isEmpty()){
                ...
                dialog.setTitleText("COMPLETE THIS ORDER ")
                dialog.setContentText("ARE YOU SURE TO PRINT THIS OFFICIAL RECEIPT?")
                ...
                dialog.setConfirmClickListener {
                    ...
                    handler.postDelayed(object :Runnable {
                        override fun run() {
                            printSomePrintable()
                            loading.isDismiss()
                            Toast.makeText(requireContext(),"PRINTING OFFICIAL RECEIPT",Toast.LENGTH_LONG).show()
                            loading.startLoading()
                            val handler = Handler()
                            handler.postDelayed(object :Runnable {
                                override fun run() {
                                    callServer?.updateStatusOrder(order_no,
                                        final.toString(), final_dp.toString(),amortization)
...
//            var total_pay = new_s.format(final).toDouble()
             ...
             amortization = final?.let { final_dp?.minus(it) }.toString()
...
//                .build())
        ...
        add(
            TextPrintable.Builder()
                .setText("CHANGE: PHP "+final_change.toString())
                .setAlignment(DefaultPrinter.ALIGNMENT_RIGHT)
                .setCharacterCode(DefaultPrinter.CHARCODE_PC1252)
                .setNewLinesAfter(2)
                .build())
