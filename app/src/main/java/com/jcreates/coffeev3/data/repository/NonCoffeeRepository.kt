...
class NonCoffeeRepository(application: Application) {
    private val dao: Non_CoffeeDao
    private val all: LiveData<List<Non_Coffee>>
    private val all2: LiveData<List<Non_Coffee>>
    private val application: Application

    init {
        val db: AppDatabase = AppDatabase.getDatabase(application)
        dao = db.getNonCoffeeDao()
        all = dao.fetchAll()
        all2 = dao.fetchAll2()
    ...
    fun getListNonCoffee(): Cursor? {
        return dao.get_list()
    }


    fun insertNonCoffeeItem(response: JSONArray) {
        Log.d("NON-COFFEE",response.toString())
        Completable.fromAction(object : Action {
            @Throws(Exception::class)
            override fun run() {
                Log.d("NON-COFFEE  ", "START")
                dao.deleteAll()
                dao.deleteAllSequence()
                val items: ArrayList<Non_Coffee> = ArrayList<Non_Coffee>()
                for (i in 0 until response.length()) {
                    val data: JSONObject = response.getJSONObject(i)
                    val item = Non_Coffee(0,
            ...
            .subscribe(object : CompletableObserver {
                ...
                override fun onComplete() {
                    Log.d("NON-COFFEE  ", "COMPLETE")
                }

                override fun onError(e: Throwable) {
                    Log.e("NON-COFFEE   ", e.message!!)
                }
            })
    }
