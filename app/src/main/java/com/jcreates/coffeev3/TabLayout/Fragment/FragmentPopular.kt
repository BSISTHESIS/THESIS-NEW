...
import com.jcreates.coffeev3.adapter.TaskAdapterShowOrderPopular
...
class FragmentPopular : Fragment() {
    ...
    private  var taskAdapter: TaskAdapterShowOrderPopular? = null
    ...
    fun uiComponent(view:View){
        ...
        taskAdapter = TaskAdapterShowOrderPopular(requireActivity().applicationContext,requireActivity())
        taskRecyclerView?.setAdapter(taskAdapter)

        popularViewModel = ViewModelProvider(this).get(PopularViewModel::class.java)

        popularViewModel?.allnotes2?.observe(viewLifecycleOwner, Observer { user->
