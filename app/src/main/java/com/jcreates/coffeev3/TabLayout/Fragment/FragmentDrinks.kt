...
import com.jcreates.coffeev3.adapter.TaskAdapterShowOrderDrinks
...
class FragmentDrinks : Fragment() {
    ...
    fun uiComponent(view:View){
        ...
        taskAdapter = TaskAdapterShowOrderDrinks(requireActivity().applicationContext,requireActivity())
        taskRecyclerView?.setAdapter(taskAdapter)

        drinksViewModel = ViewModelProvider(this).get(DrinksViewModel::class.java)

        drinksViewModel?.allnotes2?.observe(viewLifecycleOwner, Observer { user->
